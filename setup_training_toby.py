...
from setup_targets_toby import POSE_LABEL_NAMES, SPEED_LABEL_NAME, PENETROMETER_HARDNESS_LABEL_NAME

LABEL_NAMES = POSE_LABEL_NAMES + SPEED_LABEL_NAME + PENETROMETER_HARDNESS_LABEL_NAME


def csv_row_to_label(row):
    row_dict = {label: np.array(row[label]) for label in LABEL_NAMES}
    return row_dict
...
def setup_model_image(save_dir=None):
    ...
    image_processing_params = {
        'dims': (128, 128), #maybe needs changing (480,640,5)?
        'bbox': None,
        'thresh': None,
        'stdiz': False,
        'normlz': True,
...
def setup_model_labels(task_name, data_dirs, save_dir=None):
    ...
    target_label_names_dict = {
        ...
        'firmness_2d': ['penetrometer_hardness', 'speed_percentage'],
        'firmness_2d_dummy_data': ['penetrometer_hardness', 'speed_percentage'],
    }
    ...
    for data_dir in data_dirs:
        ...
        llims.append(collect_params['pose_llims']+collect_params['speed_llim']+collect_params['hardness_llim'])
        ulims.append(collect_params['pose_ulims']+collect_params['speed_ulim']+collect_params['hardness_ulim'])
        #llims.append(np.concatenate((collect_params['pose_llims'],collect_params['speed_llim'],collect_params['hardness_llim']),axis=None))
        #ulims.append(np.concatenate((collect_params['pose_ulims'],collect_params['speed_ulim'],collect_params['hardness_ulim']),axis=None))
