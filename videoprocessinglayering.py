...
data_loc = os.path.join("Datastore", "firmness_2d_dummy_data")
...
def sortimages(ssim_array,image_file,i,j,count):
    ...
    ssim_start_idx=np.where(ssim_array<0.96)
    print(ssim_start_idx[0][0]+1, ssim_min_idx[0][0]+1)
    ...
    while True:
        ...
        if flag:
            ...
            if (pos_frame <= ssim_min_idx[0][0]+1):
                putindf = False
                if (pos_frame <= ssim_min_idx[0][0]+1-5) and (pos_frame >= ssim_start_idx[0][0]):
                    putindf = True
                pos_frame = cap.get(cv2.CAP_PROP_POS_FRAMES)
