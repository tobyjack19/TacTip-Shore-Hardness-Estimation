...
from image_generator_toby import ImageDataGenerator
from models_toby import create_model
from train_model_toby import train_model
...
from setup_training_toby import setup_training, csv_row_to_label
...
def launch(args):
    ...
    for args.task, args.model in it.product(args.tasks, args.models):
        ...
        train_data_dirs = [
            os.path.join(output_dir, args.task, d) for d in args.train_dirs
        ]
        val_data_dirs = [
            os.path.join(output_dir, args.task, d) for d in args.val_dirs
        ]

        # setup save dir
        save_dir = os.path.join(output_dir, args.task, model_dir_name)
        make_dir(save_dir)
