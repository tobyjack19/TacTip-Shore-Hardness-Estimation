import os
import numpy as np

from vsp.video_stream import CvVideoCamera, CvVideoDisplay, CvVideoOutputFile, CvVideoInputFile
from vsp.detector import CvBlobDetector, CvContourBlobDetector
from vsp.tracker import NearestNeighbourTracker
from vsp.processor import CameraStreamProcessor, FileStreamProcessor, AsyncProcessor
from vsp.encoder import KeypointEncoder
from vsp.view import KeypointView

from tactile_image_processing.collect_data.setup_embodiment import setup_embodiment
from setup_targets_toby import setup_targets
from setup_targets_toby import POSE_LABEL_NAMES, SHEAR_LABEL_NAMES, OBJECT_POSE_LABEL_NAMES, SPEED_LABEL_NAME
from tactile_image_processing.utils import make_dir, save_json_obj

BASE_DATA_PATH = 'temp'

def collect_data(
    robot,
    sensor,
    targets_df,
    image_dir,
    collect_params,
):
    pose_label_names = collect_params.get('pose_label_names', POSE_LABEL_NAMES)
    shear_label_names = collect_params.get('shear_label_names', SHEAR_LABEL_NAMES)
    object_pose_label_names = collect_params.get('object_pose_label_names', OBJECT_POSE_LABEL_NAMES)
    speed_label_name = collect_params.get('speed_label_name', SPEED_LABEL_NAME)

    # start 50mm above workframe origin with zero joint 6
    robot.move_linear((0, 0, -50, 0, 0, 0))
    robot.move_joints([*robot.joint_angles[:-1], 0])

    # collect reference image
    #image_outfile = os.path.join(image_dir, 'image_0.png')
    #sensor.process(image_outfile)

    # clear object by 10mm
    clearance = (0, 0, 1, 0, 0, 0)
    robot.move_linear(np.zeros(6) - clearance)
    joint_angles = robot.joint_angles
    saved_obj_label = ''

    # ==== data collection loop ====
    for i, row in targets_df.iterrows():
        image_name = row.loc["sensor_image"]
        obj_label = row.loc["object_label"]
        pose = row.loc[pose_label_names].values.astype(float)
        shear = row.loc[shear_label_names].values.astype(float)
        obj_pose = row.loc[object_pose_label_names].values.astype(float)
        speed = row.loc[speed_label_name].values.astype(float)[0]
        robot.speed = speed
        #robot.linear_speed = row.loc[speed_label_name].values.astype(float)

        #camera = RealSensor(sensor_params)
        #for _ in range(5): camera.read()
        with AsyncProcessor(CameraStreamProcessor(
                camera=CvVideoCamera(source=1, exposure=-9, is_color=False),
                pipeline=[
                    CvBlobDetector(
                        min_threshold=31.23,
                        max_threshold=207.05,
                        filter_by_color=True,
                        blob_color=255,
                        filter_by_area=True,
                        min_area=17.05,
                        max_area=135.46,
                        filter_by_circularity=True,
                        min_circularity=0.62,
                        filter_by_inertia=True,
                        min_inertia_ratio=0.27,
                        filter_by_convexity=True,
                        min_convexity=0.60,
                    ),
                    # CvContourBlobDetector(),
                    NearestNeighbourTracker(threshold=20),
                    KeypointEncoder(),
                ],
                view=KeypointView(color=(0,255,0)),
                display=CvVideoDisplay(name='preview'),
                writer=CvVideoOutputFile(is_color=False),
        )) as p:        
            image_outfile = os.path.join(image_dir, image_name)
            p.async_process(outfile=image_outfile)
            #time.sleep(5)
            #p.async_cancel()
            #frames = p.async_result()
            #print("frames.shape = {}".format(frames.shape))

            # report
            with np.printoptions(precision=1, suppress=True):
                print(f"{i+1}/{len(targets_df.index)}: [{obj_label}] pose{pose}, shear{shear}, speed{speed}")

            clearance = (0, 0, pose[2], 0, 0, 0)
            pose[2] = 0

            # new object set reset
            if obj_label != saved_obj_label:
                saved_obj_label = obj_label
                robot.move_joints(joint_angles)
                robot.move_linear(obj_pose)
                joint_angles = robot.joint_angles

            # pose is relative to object pose
            pose += obj_pose

            # move to above new pose (avoid changing pose in contact with object)
            robot.move_linear(pose)

            # move down to offset pose
            robot.move_linear(pose + clearance)

            # collect and process tactile image
            #image_outfile = os.path.join(image_dir, image_name)
            ##sensor.process(image_outfile)

            # move above the target pose
            robot.move_linear(pose)

            p.async_cancel()
            frames = p.async_result()
            print("frames.shape = {}".format(frames.shape))

        # if sorted, don't move to reset position
        if not collect_params.get('sort', False):
            robot.move_joints(joint_angles)

    # finish 50mm above workframe origin then zero last joint
    robot.move_linear((0, 0, -50, 0, 0, 0))
    robot.move_joints((*robot.joint_angles[:-1], 0))
    robot.close()

if __name__ == "__main__":

    data_params = {
        'data_1': 50,
        'data_2': 50,
    }

    collect_params = {
        "pose_llims": (-5, 0, 3, 0, 0, -180),
        "pose_ulims": (5, 0, 4, 0, 0,  180),
        "sort": True,
        "object_poses": {
            "edge":    (0, 0, 0, 0, 0, 0),
            "surface": (-50, 0, 0, 0, 0, 0)
        }
    }

    env_params = {
        "robot": "sim",
        "stim_name": "square",
        "work_frame": (650, 0, 50, -180, 0, 0),
        "tcp_pose":   (0, 0, -85, 0, 0, 0),
        "stim_pose":  (600, 0, 12.5, 0, 0, 0),
        'show_tactile': True
    }

    sensor_params = {
        "type": "standard_tactip",
        "image_size": (256, 256)
    }

    for data_dir_name, num_poses in data_params.items():

        # setup save dir
        save_dir = os.path.join(BASE_DATA_PATH, data_dir_name)
        image_dir = os.path.join(save_dir, "sensor_images")
        make_dir(save_dir)
        make_dir(image_dir)
        save_json_obj(sensor_params, os.path.join(save_dir, 'sensor_image_params'))

        # setup embodiment
        robot, sensor = setup_embodiment(
            env_params,
            sensor_params
        )

        # setup targets to collect
        target_df = setup_targets(
            collect_params,
            num_poses,
            save_dir
        )

        # collect
        collect_data(
            robot,
            sensor,
            target_df,
            image_dir,
            collect_params
        )
