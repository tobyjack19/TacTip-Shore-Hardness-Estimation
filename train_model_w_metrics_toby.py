...
def train_model_w_metrics(
    ...
    if os.path.isfile(os.path.join(save_dir, "target-pred.csv")):
        os.remove(os.path.join(save_dir, "target-pred.csv"))
    ...
    val_loss = []
    val_acc = []
    val_pred_df_best = pd.DataFrame()
    val_targ_df_best = pd.DataFrame()
    ...
    with tqdm(total=learning_params['epochs']) as pbar:
        ...
        for epoch in range(1, learning_params['epochs'] + 1):
            ...
            if np.mean(val_epoch_loss) < lowest_val_loss:
                ...
                val_pred_df_best = val_pred_df
                val_targ_df_best = val_targ_df
    ...
    target_pred_val_df = pd.DataFrame(pd.concat([val_targ_df_best, val_pred_df_best], axis=1, keys=['targets', 'pred']))
    target_file = os.path.join(save_dir, "target-pred.csv")
    target_pred_val_df.to_csv(target_file, index=False)
