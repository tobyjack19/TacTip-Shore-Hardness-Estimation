...
def testing(args):
    ...
    for args.task, args.model, args.sample_num in it.product(args.tasks, args.models, args.sample_nums):
        ...
        # setup save dir
        save_dir = os.path.join(output_dir, args.task, runs_dir_name) #BASE_RUNS_PATH, output_dir, args.task, runs_dir_name)
        image_dir = os.path.join(save_dir, "processed_images")
        make_dir(save_dir)
        make_dir(image_dir)

        # set data and model dir
        data_dir = os.path.join(output_dir, args.task, args.train_dirs[0]) #BASE_DATA_PATH, output_dir, args.task, args.train_dirs[0])
        model_dir = os.path.join(output_dir, args.task, model_dir_name) #BASE_MODEL_PATH, output_dir, args.task, model_dir_name)
